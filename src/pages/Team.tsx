...
const TeamStructureBlock = ({ dept }: { dept: typeof departments[0] }) => {
  const isLeadValid = dept.leadName !== "none" && dept.leadName !== "";
  const leadMatch = isLeadValid ? activeCommittee.find(m => {
    if (!m.fullName || !dept.leadName) return false;
    const leadFirstName = dept.leadName.split(" ")[0].toLowerCase();
    return matchName(m.fullName, dept.leadName) || m.fullName.toLowerCase().split(" ")[0] === leadFirstName;
  }) : null;
  ...
  return (
    <div className="relative mb-32 last:mb-0 bg-[#06090f]/30 border border-white/5 rounded-[40px] p-6 sm:p-10 lg:p-16 shadow-[inset_0_0_80px_rgba(0,0,0,0.5)] overflow-hidden">
      ...
      <div className="flex justify-center mb-12 relative z-10 w-full text-center">
        <h3 className="text-3xl md:text-4xl font-heading font-extrabold text-[#E6EDF3] border-b-[3px] border-[#0F9D58] pb-3 tracking-widest uppercase inline-block">
          {dept.name}
        </h3>
      </div>
      ...
      <div className="flex flex-col items-center relative z-10 w-full">
        {allMembers.length > 0 && (
          <div className="w-full grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 md:gap-8 place-items-center relative z-10">
